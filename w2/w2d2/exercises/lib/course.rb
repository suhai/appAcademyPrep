...
class Course
  attr_reader :name, :department, :credits, :time_block, :days, :students
  ...
  def conflicts_with?(another_course)
    if self.time_block != another_course.time_block
      return false
    else
      self.days.any? { |day| another_course.days.include?(day) }
    end
  end
